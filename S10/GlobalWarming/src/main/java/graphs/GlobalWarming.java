...
public class GlobalWarming {
    ...
    public GlobalWarming(int [][] altitude, int waterLevel) {
        ...
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < n; col++) {
                ...
                detectComponents(row,col);

                if (this.components[row*n+col]==this.nbComponent) this.nbComponent++;
    ...
    private void detectComponents(int row, int col){
        ...
        if (this.altitude[row][col] <= this.waterLevel) {
            this.components[n*row + col] = -1;
        } else{

            this.components[n*row+col] = this.nbComponent;

            detectComponents(row+1,col);
            detectComponents(row-1,col);
            detectComponents(row,col+1);
            detectComponents(row,col-1);
        }
    }
    ...
    public boolean onSameIsland(Point p1, Point p2) {
        ...
        boolean notUnderWater = this.components[p1.getX()*this.altitude.length + p1.getY()] == -1;
        boolean sameComp = this.components[p1.getX()*this.altitude.length + p1.getY()] == this.components[p2.getX()*this.altitude.length + p2.getY()];

        return (!notUnderWater & sameComp);

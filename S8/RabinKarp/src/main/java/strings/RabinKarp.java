...
public class RabinKarp {
    ...
    public RabinKarp(String[] pat) {
        ...
        for (int i = 0; i < pat.length; i++) {
            long hashed = hash(pat[i], M);
            table.put(hashed,pat[i]);
        }
    ...
    public int search(String txt) { // Search for hash match in text.
        ...
        if (table.containsKey(txtHash) && table.get(txtHash).equals(txt.substring(0,M))) return 0; // Match at beginning.
        for (int i = M; i < N; i++) { // Remove leading digit, add trailing digit, check for match.
            txtHash = (txtHash + Q - RM * txt.charAt(i - M) % Q) % Q;
            txtHash = (txtHash * R + txt.charAt(i)) % Q;
             if (table.containsKey(txtHash) && table.get(txtHash).equals(txt.substring(i-M+1,i+1)))
                 return i - M + 1; // match
        }
